...
OUT_TAR_FILE=vcomponent-MotionSensor.tar.gz
...
###############################################################################
# Function to export HALIF + Binder SDK environment variables
# (Aligned to Indicator build.sh)
###############################################################################
halif_binder_env()
{
    ...
    echo -e "${BOLD}${GREEN}  BINDER_SDK_DIR=${RESET}${YELLOW}${BINDER_SDK_DIR}${RESET}"
    echo -e "${BOLD}${GREEN}  HALIF_LIB_DIR=${RESET}${YELLOW}${HALIF_LIB_DIR}${RESET}"
}
...
###############################################################################
# Install binder + halif libs/headers into build/usr
###############################################################################
...
###############################################################################
# Function to build Motion Sensor component
###############################################################################
build_motion_sensor()
{
    cmake -S . -B build \
        ...
        -DSENSOR_HALIF_INCLUDE_DIR="${VCOMPONENT_HALIF_INCLUDE_DIR}" \
    ...
    cp -f "${HALIF_LIB_DIR}/libsensor-v${VCOMPONENT_VERSION}-cpp.so" "${TOP_BUILD_DIR}/" 2>/dev/null || true
}
