...
constexpr const char* kEventNoMotion = "NO_MOTION";
...
void onControlPlaneMessage(char* key, ut_kvp_instance_t* instance, void* userData)
{
    ...
    if (!kvpGetFirstString(instance, eventKeys, &eventType, &matchedEventKey) ||
        (eventType != kEventMotion && eventType != kEventNoMotion))
    {
        LOGF_ERR(
            "%s: motion_sensor_event requires "
            "%s or %s with value MOTION or NO_MOTION payload=%s",
            logPrefix,
            kEventKey,
            kPlainEventKey,
            kvpData);
    ...
    const bool injected =
        eventType == kEventMotion
            ? implementation->injectMotionEvent()
            : implementation->injectNoMotionEvent();
    if (!injected)
    {
        LOGF_WARN(
